...
(function(){
  ...
  async function run(){
    try{
      ...
      // Support older live-news URLs already generated by the homepage.
      // Example: live-news-1-1789670879 -> second live story (0-based index).
      if(!a && /^live-news-\d+-\d+$/.test(wanted)){
        const m=wanted.match(/^live-news-(\d+)-\d+$/);
        const idx=m?Number(m[1]):-1;
        const live=list.filter(x=>x.live);
        if(idx>=0 && idx<live.length) a=live[idx];
      }

      // Support the current compact live-* index form too.
