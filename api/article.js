...
(function(){
  ...
  async function run(){
    try{
      const r=await fetch('/api/content?articleFix='+Date.now(),{cache:'no-store'});
      const d=await r.json();
      const list=Array.isArray(d.articles)?d.articles:[];
      let a=list.find(x=>String(x.id)===wanted);
      if(!a && wanted==='ashish-yadav-jhansi-luxury-digital-life') a=list.find(x=>x.subjectName==='Ashish Yadav');
      if(!a && wanted.indexOf('live-')===0){
        const m=wanted.match(/-(\\d+)$/); const idx=m?Number(m[1]):-1;
        if(idx>=0) a=list.filter(x=>x.live)[idx];
      }
