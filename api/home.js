...
function bind(){let b=ensure();if(!b)return;let old=document.getElementById('searchForm');if(old&&!old.dataset.sjFixed){let form=old.cloneNode(true);form.dataset.sjFixed='1';old.replaceWith(form);form.addEventListener('submit',function(e){e.preventDefault();search((form.querySelector('input')||{}).value||'');});}let input=document.getElementById('search');let q=new URLSearchParams(location.search).get('q');if(q&&input){input.value=q;search(q);} }
